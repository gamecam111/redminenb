...
import com.kenai.redminenb.PathSettings;
...
public class RedmineIssuePanel extends VerticalScrollPane {
    ...
    Map<String, String> urlsToFile = new HashMap<>();

    void updateTextileOutput() {
        String htmlOutput = descTextArea.getText();
        for (Map.Entry<String, String> entry : urlsToFile.entrySet()) {
            htmlOutput = htmlOutput.replace(entry.getKey(), entry.getValue());
            //htmlOutput = htmlOutput + " " + entry.getValue() + " " + "!" + entry.getValue() + "!";
        }

        htmlOutputLabel.setTextileText(htmlOutput);
    }
                ...
                if (!redmineIssue.isNew()) {
                    ...
                    if (issue.getAttachments() != null) {
                        for (Attachment ad : issue.getAttachments()) {
                            ...
                            File dirs = new File(PathSettings.cacheDir + issue.getId());
                            dirs.mkdirs();
                            File file = new File(PathSettings.cacheDir + issue.getId() + "//" + ad.getFileName());
                            try ( FileOutputStream fos = new FileOutputStream(file)) {
                                redmineIssue.getRepository().getAttachmentManager().downloadAttachmentContent(ad, fos);
                            } catch (Exception e) {
                                //do nothing
                            }

                            urlsToFile.put(ad.getFileName(), "file:///" + PathSettings.cacheDir + issue.getId() + "//" + ad.getFileName());
